...
# 绘制某一类别的PR曲线
specific_class = 'SJP-ED'
# specific_class = '2Mild'
# specific_class = '3Medium'
# specific_class = '4Severe'
# specific_class = '5Extreme'
# 二分类标注
y_test = (df['标注类别名称'] == specific_class)
# y_test = (df['Label category name'] == specific_class)

# 二分类预测置信度
# y_score = df['开花成熟-中旱-预测置信度']
y_score = df['RJ-ED-预测置信度']
...
plt.plot([0, 0], [0, 1], ls="--", c='.3', linewidth=3, label='Stochastic model')
...
plt.title('{} PR curve  AP:{:.3f}'.format(specific_class, AP))
plt.xlabel('Recall')
plt.ylabel('Precision')
plt.legend()
plt.grid(True)
# plt.savefig('{}-PR曲线.pdf'.format(specific_class), dpi=120, bbox_inches='tight')
# plt.savefig('{}-PR-curve.pdf'.format(specific_class), dpi=120, bbox_inches='tight')
# plt.show()
...
plt.savefig('所有类别PR曲线.pdf'.format(specific_class), dpi=120, bbox_inches='tight')
plt.show()
